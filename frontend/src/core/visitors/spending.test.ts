...
const mkVisitor = (overrides?: Partial<Visitor>): Visitor => ({
  id: 1,
  position: { x: 0, y: 0 },
  prevPos: null,
  inAttraction: false,
...
describe('spendingPerTick', () => {
  it('is deterministic (same input => same output)', () => {
    const v = mkVisitor({ fear: 42, happiness: 77 });
    expect(spendingPerTick(v)).toBe(spendingPerTick(v));
    expect(spendingPerTick(v)).toBe(spendingPerTick({ ...v }));
  });

  it('stops spending when happiness is at or below the unhappy threshold', () => {
    expect(spendingPerTick(mkVisitor({ happiness: UNHAPPY_SPEND_STOP, fear: 0 }))).toBe(0);
    expect(spendingPerTick(mkVisitor({ happiness: UNHAPPY_SPEND_STOP - 1, fear: 0 }))).toBe(0);
  });

  it('stops spending when fear is at or above the panic threshold', () => {
    expect(spendingPerTick(mkVisitor({ fear: FEAR_PANIC_THRESHOLD, happiness: 100 }))).toBe(0);
    expect(spendingPerTick(mkVisitor({ fear: FEAR_PANIC_THRESHOLD + 1, happiness: 100 }))).toBe(0);
  });
  ...
  it('happier visitors spend more (holding fear constant)', () => {
    ...
    const lowHappy = mkVisitor({ happiness: HAPPY_SPEND_BOOST_START, fear: safeFear });
    const highHappy = mkVisitor({ happiness: 100, fear: safeFear });
  ...
  it('fear increases spending up to the boost cap (holding happiness constant)', () => {
    ...
    const lowFear = mkVisitor({ fear: FEAR_SPEND_BOOST_START, happiness: happy });
    const midFear = mkVisitor({
      fear: Math.floor((FEAR_SPEND_BOOST_START + FEAR_SPEND_BOOST_CAP) / 2),
      happiness: happy,
    });
    const capFear = mkVisitor({ fear: FEAR_SPEND_BOOST_CAP, happiness: happy });
  ...
  it('fear bonus is capped at MAX_FEAR_BONUS_PER_TICK (below panic)', () => {
    ...
    const atCap = mkVisitor({ fear: FEAR_SPEND_BOOST_CAP, happiness: happy });
    const aboveCap = mkVisitor({ fear: fearAtOrAboveCapButBelowPanic, happiness: happy });
  ...
  it('maximum happiness produces at most MAX_HAPPY_BONUS_PER_TICK bonus (not unbounded)', () => {
    ...
    const justAtBoostStart = mkVisitor({ happiness: HAPPY_SPEND_BOOST_START, fear });
    const maxHappy = mkVisitor({ happiness: 100, fear });
...
describe('totalSpendingPerTick', () => {
  it('sums spendingPerTick across visitors', () => {
    const v1 = mkVisitor({ id: 1, fear: 0, happiness: 100 });
    const v2 = mkVisitor({ id: 2, fear: FEAR_PANIC_THRESHOLD, happiness: 100 }); // should spend 0
    const v3 = mkVisitor({ id: 3, fear: FEAR_SPEND_BOOST_CAP, happiness: 100 });
